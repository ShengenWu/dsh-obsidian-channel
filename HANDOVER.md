...
## 0. 关于「GitHub Ripple」的说明（重要）

本会话**没有探索过任何名为 Ripple 的仓库或内容**。如果交接双方提到的是一个具体的
GitHub 项目（仓库名或链接），请把它补充给原会话的 agent（或直接补进本文档第 4 节）。
原会话中不存在与 Ripple 相关的结论，请勿沿用任何「本会话看过 Ripple」的说法。
